...
$host = "host=52.206.226.157";
$port = "port=5432";
$dbname = "dbname=bugtrackerdb";
$credentials = "user=bugtracker_user password=Techtree@123";
define('HOST_NAME', 'http://test.techtreeit.in/JIRA_Reports/');
/* * **** local Credentials **** */

/*  $host        = "host=127.0.0.1";
  $port        = "port=5432";
  $dbname      = "dbname=jira_july12";
  $credentials = "user=mithun password=123456"; */

/*********** IMAGE IMPORT URL ******************/
$GLOBALS['jira_url'] = 'http://jirawork.techtreeit.com';
...
$sqlQueries = array(
    ...
    'total-time-spent-assignee-last-month' => array(
        ...
        "query"=>"select E.author as assignee, C.parent_name as parent_name,C.parent_id as parent_id,TO_CHAR((sum(E.timeworked) || ' second')::interval, 'HH24:MI') as timespent 
                    from 
                    worklog E,
                    cwd_membership C,
                    app_user B,
		    cwd_user D
                    where %s and D.id = C.child_id and C.parent_id not in (10002,10823,10824,10910,10911,10912,10913,10000) and D.user_name = B.lower_user_name
    ...
    'allprojects' => array(
        "query" => 'select id,pname from project %s',
    ),
    ...
    'epic-user-issues' => array(
        'query' => "SELECT *,(SELECT pname from project where id = (SELECT project from jiraissue where id= '%s')) as project,(SELECT summary FROM jiraissue WHERE id = '%s') as summary,(SELECT project from jiraissue where id= '%s') as pid FROM jiraissue WHERE id IN (select destination from issuelink where source = '%s' ) and jiraissue.assignee='%s'"
    ),
    'task-plan' => array(
...
//'query'=>"SELECT distinct(assignee) from jiraissue",
    ...
    'task-no-duedate' => array(
        ...
        'columns' => array('gotojira' => 'Jira Link', 'summary' => 'Summary','issuetypename' => 'Issue Type', 'assignee' => 'Assignee', 'reporter' => 'Report To',
            'created' => 'Created At'),
    ),
    'user-task-time-spent-individual' => array(
        ...
        */
        'query' => "SELECT project,W.timeworked as timespent,A.timeoriginalestimate as timeoriginalestimate,pname,W.startdate,W.updated as resolutiondate
                        FROM jiraissue as A 
                            inner join project B on A.project=B.id 
                            inner join worklog W on A.id=w.issueid 
    ...
    'sprint-all-projects'=>array(
        "query"=>'select 
                    ...
                    where 
                        p.id = i.project and
                        c.issue = i.id and CAST(c.stringvalue AS BIGINT) = s."ID" and c.customfield = 10002 %s
    ...
    'projects-sprint'=>array(
        "query"=>'select pname,sid,sname,start_date,end_date,complete_date,sum(nb_task) as nb_task,concat(TO_CHAR((sum(final.totalestimate) || \' second\')::interval, \'HH24:MI\'), \' hrs\') as totalestimate,concat(TO_CHAR((sum(timespent) || \' second\')::interval, \'HH24:MI\'), \' hrs\') as timespent, status 
		from ((select 
                        p.pname as pname,
                        s."ID" as sid,
                        s."NAME" as sname, 
                        ...
                        sum(i.timeoriginalestimate) as totalestimate,
                        sum(i.timespent) as timespent,
                    ...
                    where 
                        ...
                        c.issue = il.source and CAST(c.stringvalue AS BIGINT) = s."ID" and c.customfield = 10002 and i.id = il.destination
		...
		UNION (select 
                    ...
                    where 
                        p.id = %s and p.id = i.project and
                        c.issue = i.id and CAST(c.stringvalue AS BIGINT) = s."ID" and c.customfield = 10002 and i.issuetype != \'5\'
                    group by s."ID",s."NAME",p.pname,s."START_DATE",s."END_DATE",s."COMPLETE_DATE"
                    order by s."START_DATE" ASC)) as final 
group by sid,pname,sname,start_date,end_date,complete_date,status
order by sid;',
        "query_chart"=>'select pname, sid, sname, sum(final.nb_task) as nb_task, SUM(final.resolved_issues) as resolved_issues 
		from ( (select 
                        p.pname as pname, s."ID" as sid, s."NAME" as sname, count(i.id) as nb_task, SUM(CASE WHEN (i.issuestatus IN (\'5\',\'6\',\'10001\')) THEN 1 ELSE 0 END ) as resolved_issues
                    from
                        "customfieldvalue" as c, 
                        "jiraissue"  as i, 
                        "project" as p, 
		...
		UNION (select 
                    ...
                    where 
                        p.id = %s and p.id = i.project and
                        c.issue = i.id and CAST(c.stringvalue AS BIGINT) = s."ID" and c.customfield = 10002 and i.issuetype != \'5\'
                    group by s."ID",s."NAME",p.pname)) as final 
                    group by sid,final.pname,final.sname order by sid;',
        "small_query"=>'select 
                        p.id as pid,
                        s."ID" as id,
                        s."NAME" as sname                        
    ...
    'sprint-timespent-rolewise'=>array(
        ...
        'query'=>'select 
                        (select TO_CHAR((sum(i.timespent) || \' second\')::interval, \'HH24:MI\') as spenttester
                            ...
                            where CM.parent_id not in (10002,10823,10824,10910,10911,10912,10913,10000) and CM.child_name=U.user_key
	...
	'total-time-spent-assignee-last-month-leave-holiday' => array(

            "query"=>"select E.author as assignee,C.parent_id as parent_id, C.parent_name as parent_name,TO_CHAR((sum(E.timeworked) || ' second')::interval, 'HH24:MI') as timespent
                        from
                        worklog E,
                        cwd_membership C,
                        app_user B,
                        cwd_user D
                        where D.id = C.child_id and C.parent_id in (10910,10911,10912,10913) and D.user_name = B.lower_user_name
                        and E.author = B.user_key
                        %s
                        group by E.author,B.id,C.id,D.id
                        order by E.author",
...
$permission = array(
        ...
        "epic_report" => "Epic Wise Report", 
        "sprint_report" => "Sprint Wise Report", 
        "user_permission" => "User Permission",
        "change-password"=>"Change Password"
    );
    //2 => array('task-in-progess' => "Tasks in Progress", "active-users" => "Active users", "users-no-task-today" => "users-no-task-today", "task-no-duedate" => "Tasks without Due Date", "unestimated_task" => "unestimated_task", "holiday" => "holidays", "project_worklogs" => "project worklogs", "total-time-spent-assignee-last-month" => "user worklogs", "epic_report" => "epic wise report", "sprint_report" => "sprint wise report"),
    //3 => array('task-in-progess' => "Tasks in Progress", "active-users" => "Active users", "users-no-task-today" => "users-no-task-today", "task-no-duedate" => "Tasks without Due Date", "unestimated_task" => "unestimated_task","epic_report" => "epic wise report", "sprint_report" => "sprint wise report"),
    //4 => array('task-in-progess' => "Tasks in Progress", "active-users" => "Active users", "users-no-task-today" => "users-no-task-today", "task-no-duedate" => "Tasks without Due Date", "unestimated_task" => "unestimated_task","epic_report" => "epic wise report", "sprint_report" => "sprint wise report"),
    //5 => array('task-in-progess' => "Tasks in Progress", "active-users" => "Active users", "users-no-task-today" => "users-no-task-today", "task-no-duedate" => "Tasks without Due Date", "unestimated_task" => "unestimated_task"),
//);
