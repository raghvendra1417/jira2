...
// create a simple 2-dimensional array
        ...
        while($retPostData2 = pg_fetch_object($retPostData)){
	        ...
	        if($nodays <= 32){
                ...
                for (;strtotime($fromDate) <= strtotime($toDate); ) { 
                	...
                	if(date('N',strtotime($fromDate)) > 5){ $fromDate = date('d-m-Y', strtotime($fromDate.$incTime)); continue; } //no log on saturday & Sundays
    ...
    header('Content-Disposition: attachment;filename="'.date("d M",strtotime($fromDateWD))." to ".date("d M",strtotime($toDateWD))."_(". date('Y').')_Mobility.xls"');
