...
$host = "host=52.206.217.66";
$port = "port=5432";
$dbname = "dbname=bugtrackerdb";
$credentials = "user=bugtracker_user password=Techtree@123";
define('HOST_NAME', 'http://test.techtreeit.in/JIRA_Reports_mobile/');
/* * **** local Credentials **** */
...
/*********** IMAGE IMPORT URL ******************/
$GLOBALS['jira_url'] = 'http://jiraplay.techtreeit.com';
...
//'query'=>"SELECT distinct(assignee) from jiraissue",
    ...
    'projects-sprint'=>array(
        ...
        "query_chart"=>'select 
                        ...
                        s."COMPLETE_DATE" as complete_date,
                        count(i.id) as nb_task,
                        SUM(CASE WHEN (i.issuestatus IN (\'5\',\'6\',\'10001\')) THEN 1 ELSE 0 END ) as resolved_issues,
                        concat(TO_CHAR((sum(i.timeoriginalestimate) || \' second\')::interval, \'HH24:MI\'), \' hrs\') as totalestimate,
                        concat(TO_CHAR((sum(i.timespent) || \' second\')::interval, \'HH24:MI\'), \' hrs\') as timespent,
                        (CASE WHEN(s."CLOSED") THEN \'CLOSED\' ELSE \'IN PROGRESS\' END)  as status
                    from
	...
	'total-time-spent-assignee-last-month-leave-holiday' => array(
            ...
            "query"=>"select E.author as assignee,C.parent_id as parent_id, C.parent_name as parent_name,TO_CHAR((sum(E.timeworked) || ' second')::interval, 'HH24:MI') as timespent
                        ...
                        app_user B,
                        cwd_user D
                        where D.id = C.child_id and C.parent_id not in (10002,10823,10824,10910,10911,10912,10913) and D.user_name = B.lower_user_name
                        and E.author = B.user_key
                        %s
                        group by E.author,B.id,C.id,D.id
                        order by E.author",
