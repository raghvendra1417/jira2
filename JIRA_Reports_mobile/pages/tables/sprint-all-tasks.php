<?php 
	...
	include './../../config.php';
        ...
        $actionsData = array(
                'sprint-all-tasks'=>array('cpage'=>'Epic Wise Report <span style="font-size: 10px;">(Active Projects)</span>','pbreadcrumb'=>array())
                
            );
        
	  if(array_key_exists($action,$actionsData) && $action == 'sprint-all-tasks') {
               
               $ismysqlQuery = 0; 
               $sql = $sqlQueries['sprint-all-tasks']['query'];
		
               $sql = sprintf($sql,$_REQUEST['sid'],$_REQUEST['id']);
           ...
           //while($rowretData = pg_fetch_object($retData)){ 
               ...
               //$timespent += $rowretData->timespent;
              //$timees += $rowretData->estimate;
           //}
           #echo "$timees $timespent";exit;
	//project data
