<?php 
	...
	include './../../config.php';
        ...
        if(!in_array("epic_report",$_SESSION['role_permission'])){
        ...
        $actionsData = array(
                'all-projects'=>array('cpage'=>'Epic Wise Report <span style="font-size: 10px;">(Active Projects)</span>','pbreadcrumb'=>array())
                
            );
        
	  if(array_key_exists($action,$actionsData) && $action == 'all-projects') {
               ...
               $sql = $sqlQueries[$action]['query'];
                ...
                if($_SESSION['highest_role'] !=1){
                    if(!empty($projectIds)){
                    $sqlStrAllProjects .= ' and B.id in ('.trim(implode(",", $projectIds)).') ';
                    }else{
                    $sqlStrAllProjects .= ' and B.id in (0)';
                    }
                }
	...
	//project data
		...
		$sqlProject = $sqlQueries['all-projects']['query'];
                ...
                if($_SESSION['highest_role'] !=1){
                    if(!empty($projectIds)){
                    $sqlStrAllProjects1 .= ' and B.id in ('.trim(implode(",", $projectIds)).') ';
                    }else{
                    $sqlStrAllProjects1 .= ' and B.id in (0) ';
                    }
                }

                $sqlProject = sprintf($sqlProject,$sqlStrAllProjects1);
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1>
                        <?php echo $actionsData[$action]['cpage'];?> <?php echo isset($projectName)?"- ".$projectName:""; ?>
                                ...
                                <div class="box-header">
                                    ...
                                    <div style="width:33%;float: left;">
                                        ...
                                        <?php while($rowProject = pg_fetch_object($retProject)){ ?>    
                                                <option value="<?php echo $rowProject->project; ?>" <?php echo isset($_REQUEST['id']) && $rowProject->project == $_REQUEST['id']?"selected":""; ?>><?php echo $rowProject->pname; ?></option>
                                        <?php } ?>
                                        </select>
                                    </div>
						...
						<?php foreach($sqlQueries[$action]['columns'] as $keycol => $columns) { ?>
						    ...
						    <td>
                                                        <?php if ($action == 'all-projects' && $keycol=='pname') { ?>
                                                            <a href="./../../pages/tables/epic_data.php?view=epic-data&id=<?php echo $rowretData->project;?>">            
                                                        <?php }?>
		...
		$('#project-select').change(function(){
			//alert($('#project-select').val());
                    if($('#project-select').val() != ''){
                        ...
                        window.location = "./../../pages/tables/epic_data.php?view=epic-data&id="+$('#project-select').val();
                    }else{
